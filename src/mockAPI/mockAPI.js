...
export function getCharacter(id) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      if (retCharacter === undefined) {
        reject(new Error("No se encontró el personaje."));
      } else {
        resolve(retCharacter);
      }
    }, 2000);
  });
}
...
export function getCharactersByCategory(category) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      if (retCategory.length === 0) {
        reject(new Error("No se encontró la categoría."));
      } else {
        resolve(retCategory);
      }
    }, 2000);
